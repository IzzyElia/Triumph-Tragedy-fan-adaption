...
{
    [ForceInitialize]
    public class InitialUnitsAction : PlayerAction
    {
        static InitialUnitsAction()
        {
            PlayerAction.RegisterPlayerActionType<InitialUnitsAction>();
        }

        private (int iTile, byte unitType, int iCountry, byte pipsToAdd)[] _builds = Array.Empty<(int, byte, int, byte)>();

        public InitialUnitsAction() {}
        ...
        {
            (int iTile, byte unitType, int iCountry, byte pipsToAdd)[] oldBuilds = _builds;
            _builds = new (int iTile, byte unitType, int iCountry, byte pipsToAdd)[oldBuilds.Length + 1];
            oldBuilds.CopyTo(_builds, 0);
            _builds[^1] = ((int iTile, byte unitType, int iCountry, byte pipsToAdd))parameters[0];
        }
